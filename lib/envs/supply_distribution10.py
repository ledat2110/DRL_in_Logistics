...
class SupplyDistribution (gym.Env):
    """
    The supply distribution environment
    """

    def __init__(self, n_stores=3, cap_truck=2, prod_cost=1, max_prod=3,
                 store_cost=np.array([0, 2, 0, 0]), truck_cost=np.array([3, 3, 0]),
                 cap_store=np.array([50, 10, 10, 10]), penalty_cost=1, price=2.5, gamma=1,
                 max_demand=3, episode_length=25):
        """
    ...
    def reset(self):
        ...
        self.state = np.hstack((self.s.copy(), self.demand.copy(), self.demand_old.copy()))
    ...
    def step(self, action):
        ...
        # Define state
        self.state = np.hstack((self.s.copy(), self.demand.copy(), self.demand_old.copy()))  # hstack: Stack arrays in sequence horizontally
